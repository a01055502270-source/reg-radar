...
export default function Page(){
  var pg=useState("dashboard");var setPg=pg[1];pg=pg[0];
  var sel=useState(null);var setSel=sel[1];sel=sel[0];
  var regs=useState([]);var setRegs=regs[1];regs=regs[0];
  var ana=useState({});var setAna=ana[1];ana=ana[0];
  var anaing=useState(null);var setAnaing=anaing[1];anaing=anaing[0];
  var anaErr=useState({});var setAnaErr=anaErr[1];anaErr=anaErr[0];
  var dept=useState("리스크관리부");var setDept=dept[1];dept=dept[0];
  var sideState=useState(false);var setSide=sideState[1];var side=sideState[0];
  var rptGen=useState(false);var setRptGen=rptGen[1];rptGen=rptGen[0];
  var rpt=useState(null);var setRpt=rpt[1];rpt=rpt[0];
  var rptP=useState("2026년 3월");var setRptP=rptP[1];rptP=rptP[0];
  var mob=useState(false);var setMob=mob[1];mob=mob[0];
  ...
  // 규제 데이터 로드
  useEffect(function(){
    fetch("/api/regulations?limit=50")
  ...
  function go(p){setPg(p);setSel(null);setSide(false);}
      ...
      <aside style={{width:mob?260:220,background:T.navy,display:"flex",flexDirection:"column",flexShrink:0,...(mob?{position:"fixed",left:side?0:-270,top:0,bottom:0,zIndex:100,transition:"left .25s",boxShadow:side?"4px 0 20px rgba(0,0,0,.3)":"none"}:{})}}>
        <div style={{padding:"20px 18px 16px",borderBottom:"1px solid "+T.navyLight}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div><div style={{color:"#E8ECF4",fontSize:14,fontWeight:700}}>규제레이더</div><div style={{color:"#6B7FA3",fontSize:10,fontFamily:MO}}>v3.1</div></div>
          </div>
        </div>
        ...
        <div style={{flex:1,overflow:"auto",padding:mob?"16px":"20px 24px"}}>
          ...
          {!loading&&pg==="dashboard"&&!sel&&<div>
            <div style={{display:"grid",gridTemplateColumns:mob?"1fr 1fr":"repeat(4,1fr)",gap:mob?8:14,marginBottom:mob?14:20}}>
              {[["신규",regs.length,"전체",null],["긴급",crit,"즉시 검토",T.red],["내 부서",myRegs.length,dept,T.primary],["분석",Object.keys(ana).length,"/ "+regs.length+"건",null]].map(function(arr,i){
                return <div key={i} style={{background:T.surface,border:"1px solid "+T.border,borderRadius:T.R,padding:mob?"12px":"16px 18px"}}>
                  <div style={{fontSize:11,color:T.textTer,marginBottom:4}}>{arr[0]}</div>
                  <div style={{fontSize:mob?22:26,fontWeight:700,color:arr[3]||T.text}}>{arr[1]}</div>
                  <div style={{fontSize:10,color:T.textTer,marginTop:2}}>{arr[2]}</div>
                </div>;
              })}
            </div>
            {crit>0&&<div style={{background:T.redBg,border:"1px solid "+T.redBd,borderRadius:T.R,padding:"10px 14px",marginBottom:14,fontSize:12,color:T.red}}><b>긴급</b> — {crit}건 즉시 검토 필요</div>}
            {myRegs.length===0&&<div style={{textAlign:"center",padding:"40px 0",color:T.textTer}}>규제 데이터가 없습니다. GitHub Actions에서 스캔을 실행하세요.</div>}
            {myRegs.length>0&&<div>
              <div style={{fontSize:14,fontWeight:700,marginBottom:10}}>{dept} 관련</div>
              <div style={{background:T.surface,border:"1px solid "+T.border,borderRadius:T.R,overflow:"hidden"}}>
                {myRegs.map(function(r,i){
                  return <div key={r.id} onClick={function(){setSel(r.id);}} style={{padding:mob?"12px 14px":"14px 18px",cursor:"pointer",borderBottom:i<myRegs.length-1?"1px solid "+T.borderL:"none"}}>
                    ...
                    <div style={{fontSize:11,color:T.textTer,marginTop:3}}>{r.category} · {r.status}</div>
                  </div>;
                })}
              </div>
            </div>}
          ...
          {!loading&&pg==="feed"&&!sel&&<div>
            <div style={{background:T.surface,border:"1px solid "+T.border,borderRadius:T.R,overflow:"hidden"}}>
              {regs.map(function(r,i){
                return <div key={r.id} onClick={function(){setSel(r.id);}} style={{padding:mob?"12px 14px":"12px 18px",cursor:"pointer",borderBottom:i<regs.length-1?"1px solid "+T.borderL:"none"}}>
              ...
              {regs.length===0&&<div style={{padding:32,textAlign:"center",color:T.textTer}}>규제 데이터 없음. GitHub Actions에서 스캔을 실행하세요.</div>}
            </div>
          </div>}
          ...
          {!loading&&sel&&det&&<div>
            ...
            <div style={{background:T.surface,border:"1px solid "+T.border,borderRadius:T.R,overflow:"hidden"}}>
              <div style={{padding:mob?"16px":"20px 24px",borderBottom:"1px solid "+T.borderL}}>
                <div style={{display:"flex",gap:6,marginBottom:8,flexWrap:"wrap"}}><Badge s={det.severity}/><SrcB id={det.source}/></div>
                <h2 style={{margin:"0 0 8px",fontSize:mob?15:18,fontWeight:700,lineHeight:1.4}}>{det.title}</h2>
                <div style={{display:"flex",gap:12,fontSize:12,color:T.textSec,flexWrap:"wrap"}}><span>일자: <b>{(det.publishedAt||"").slice(0,10)}</b></span><span>분류: <b>{det.category}</b></span><span>상태: <b>{det.status}</b></span></div>
                ...
                {det.context&&<div style={{marginTop:12,padding:"10px 14px",background:T.surfaceAlt,borderRadius:T.R,border:"1px solid "+T.borderL,fontSize:12,color:T.textSec,lineHeight:1.6}}>{det.context}</div>}
              </div>
              <div style={{padding:mob?"16px":"20px 24px"}}>
                {!ai&&!isA&&!er&&<div style={{textAlign:"center",padding:"20px 0"}}><p style={{fontSize:13,color:T.textSec,marginBottom:12}}>규제 영향도 및 대응방안을 분석합니다.</p><Btn primary onClick={function(){doAnalyze(det);}}>분석 실행</Btn></div>}
                {!ai&&!isA&&er&&<div><div style={{background:T.redBg,border:"1px solid "+T.redBd,borderRadius:T.R,padding:"14px 16px",marginBottom:12}}><div style={{fontSize:12,fontWeight:700,color:T.red,marginBottom:4}}>실패</div><div style={{fontSize:12,color:T.red,lineHeight:1.6,wordBreak:"break-all"}}>{er}</div></div><div style={{textAlign:"center"}}><Btn primary onClick={function(){doAnalyze(det);}}>재시도</Btn></div></div>}
                {isA&&<Timer/>}
                {ai&&<div>
                  {ai.summary&&<div style={{marginBottom:18}}><div style={{fontSize:11,fontWeight:700,color:T.textTer,marginBottom:6}}>요약</div><p style={{fontSize:13,color:T.text,lineHeight:1.7,margin:0,padding:"12px 14px",background:T.surfaceAlt,borderRadius:T.R,borderLeft:"3px solid "+T.primary}}>{ai.summary}</p></div>}
                  {ai.impact&&<div style={{marginBottom:18}}><div style={{fontSize:11,fontWeight:700,color:T.textTer,marginBottom:6}}>실무 영향</div><p style={{fontSize:13,color:T.textSec,lineHeight:1.7,margin:0}}>{ai.impact}</p></div>}
                  {ai.changes&&ai.changes.length>0&&<div style={{marginBottom:18}}><div style={{fontSize:11,fontWeight:700,color:T.textTer,marginBottom:6}}>변경사항</div><div style={{border:"1px solid "+T.border,borderRadius:T.R,overflow:"hidden"}}>{ai.changes.map(function(c,i){return <div key={i} style={{display:"flex",padding:"10px 14px",borderBottom:i<ai.changes.length-1?"1px solid "+T.borderL:"none",fontSize:13,color:T.textSec,lineHeight:1.5,gap:10}}><span style={{color:T.primary,fontWeight:700,fontFamily:MO,fontSize:11,minWidth:20}}>{i+1}</span><span>{c}</span></div>;})}</div></div>}
                  {ai.actions&&ai.actions.length>0&&<div style={{marginBottom:18}}><div style={{fontSize:11,fontWeight:700,color:T.red,marginBottom:6}}>조치 필요</div><div style={{background:T.redBg,border:"1px solid "+T.redBd,borderRadius:T.R,overflow:"hidden"}}>{ai.actions.map(function(c,i){return <div key={i} style={{display:"flex",padding:"10px 14px",borderBottom:i<ai.actions.length-1?"1px solid "+T.redBd:"none",fontSize:13,color:T.red,lineHeight:1.5,gap:10}}><span style={{fontWeight:700,fontFamily:MO,fontSize:11,minWidth:20}}>{i+1}</span><span>{c}</span></div>;})}</div></div>}
          ...
          {pg==="settings"&&<div style={{background:T.surface,border:"1px solid "+T.border,borderRadius:T.R,padding:mob?16:24}}>
            ...
            <p style={{fontSize:13,color:T.textSec}}>알림 및 사용자 설정은 추후 업데이트됩니다.</p>
          </div>}
        </div>
      </main>
